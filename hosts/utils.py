...
def ssh2_deploy(host,cmd):  
    try:
        out_info = ''
        ssh = paramiko.SSHClient()
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        
        ssh.connect(host,username='root', password='rootroot',port = 22)
        logging.info(cmd)
        stdin, stdout, stderr = ssh.exec_command(cmd)
...
def ssh2_deploy_log(host,cmd):
    try:
        #out_info = []
        ssh = paramiko.SSHClient()
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())

        ssh.connect(host,username="root", password="rootroot",port = 22,timeout=2000)
        logging.info(cmd)
        stdin, stdout, stderr = ssh.exec_command(cmd)
...
def sftp2(hostname,localfile,remotefile):
    try:
        
        username_deploy = "root"
        password_deploy = "rootroot"
